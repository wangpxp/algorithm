...
public class InsertionSort {
    ...
    public static <T extends Comparable> void insertionSort_1(T[] arr) {
    ...
    // 在近乎排好序的数组里几乎是最快的算法
    public static <T extends Comparable> void insertionSort_2(T[] arr) {
        for (int i = 0; i < arr.length; i++) {
            //查找arr[i]应该插入的位置
            T e = arr[i];
            int j; // 保存arr[i]插入的位置
    ...
    // 因此使用也不是很多，不如快排有优势
    // 二分法的难点是没有那个点，找到返回条件
    public static <T extends Comparable> void insertionSort_3(T[] arr) {
        for (int i = 0; i < arr.length; i++) {
            // 用二分法查找arr[i]应该插入的位置
            T e = arr[i];
